...
function buildDataSet(data, numInputs, numOutputs) {
    ...
    if(networkType === 'RBF') {
        annCon.eta = 0.1;
        annCon.centerCount = Number(args[5]) || 50;
        network = new RBFController(annCon, data);
        filename += annCon.centerCount;
    ...
    for (var i = 0; i < outs.length; i++) {
        storeData(filename, numOutputs + ',' + outs[i].join(',') + ',' + exps[i]);
    }
    ...
    (function store(i) {
        if (i === 0) {
            ...
            storeData('results/' + filename, precision, function() {
                store(i + 1);
            });
        } else if (i === 1) {

            storeData('results/' + filename, recall, function() {
                store(i + 1);
            });
        } else if (i - 2 < confusionMatrix.length) {
            storeData('results/' + filename, confusionMatrix[i - 2], function() {
                store(i + 1);
            });
        }
    })(0);
