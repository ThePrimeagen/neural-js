...
var EvolutionaryRBFController = function(configuration) {
    ...
    this._fitnessFn = fitnessFn(this, configuration.validation);
    var rbfs = this._network._outputs[0].w.length;

    this._basePop = getBasePopulation(rbfs, configuration.t[1].length);
    configuration.evoOptions.basePopulation = this._basePop;
    configuration.evoOptions.fitnessFn = this._fitnessFn;
    this._selector = configuration.evo(configuration.evoOptions);
...
EvolutionaryRBFController.prototype = {
    train: function(inputs, expectedOutputs) {
        ...
        this._loop();
        this._network.train(inputs, expectedOutputs);
    },
    test: function(inputs) {
        return this._network.test(inputs);
    },
    getError: function() {
        return this._network.getError();
    },
