...
ANFISNetwork.prototype = _.assign(ANFISNetwork.prototype, {
    ...
    _feedForward : function () {
        ...
        for (var i = 0; i < this._layers[3].length; i++) {
            var inputValues = [];
            var inPntrs = this._layers[3][i].inPntrs;
            for (var j = 0; j < inPntrs.length; j++) {
                inputValues.push(inPntrs[j].output);
            }
            this._layers[3][i].output = NetworkMath.normalizeDataFunction(inputValues[i], inputValues);
        }
        ...
        for (var i = 0; i < this._layers[4].length; i++) {
            ...
            neuron.output = neuron.inPntrs[0].output * NetworkMath.takagiSugenoFunction(this._inputs, neuron.coefficients);
        }

        //layer 5
        for (var i = 0; i < this._layers[5].length; i++) {
            var sum = 0;
            for(var j = 0; j < this._layers[5][i].inPntrs.length; j++) {
                sum += this._layers[5][i].inPntrs[j].output;
            }

            this._layers[5][i].output = sum;
        }
    }
});

ANFISNetwork.createByClusters = function(configuration, inputs) {
    var columns = rxStats.transform(inputs);   
