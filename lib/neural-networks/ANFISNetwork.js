...
ANFISNetwork.prototype = _.assign(ANFISNetwork.prototype, {
    ...
    _backpropErrorOutput: function(expectedOutputs) {
        ...
        // ---------------------------------------------------
        // Attempting to implement: http://www.csee.wvu.edu/classes/cpe521/presentations/ANFIS.pdf
        // Skeptical of this performance
        var errEta = this.getOutput()[0] - expectedOutputs[0];
        errEta *= 2 * this.eta;
        var sumCmulV = 0;

        for (var i = 0; i < this._c.length; i++) {
            this._c[i] += errEta;
            sumCmulV += this._c[i] * this._normLayer[i].output;
        }

        for (var i = 0; i < this._ifLayer.length; i++) {
            var n = this._ifLayer[i];
            n.center[0] += errEta * this._normLayer[i].output * (this._c[i] - sumCmulV) * ((n.input - n.center[0]) / (n.sigma * n.sigma));
        }

        for (var i = 0; i < this._ifLayer.length; i++) {
            var n = this._ifLayer[i];
            n.sigma += errEta * this._normLayer[i].output * (this._c[i] - sumCmulV) * (Math.pow(n.input - n.center[0], 2) / Math.pow(n.sigma, 3));
        }
    },
