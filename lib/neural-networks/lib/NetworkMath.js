...
var NetworkMath = {
    ...
    transposeFunction: function(matrix) {
        var arr = [];
    ...
    anfisLearningFunction: function(xcurrent, sNext, rowSelection, expectedTranspose) {
        var result = xcurrent;
        var sub1 = matrix.subtract(expectedTranspose, matrix.mutltiply(matrix.transform(rowSelection), xcurrent));
        var sub2 = matrix.multiply(sNext, rowSelection);
        var sub3 = matrix.multiply(sub2, sub1);

        return result + sub3[0][0];
    }
