 ...
 * @return {RBFNetwork}
 */
RBFNetwork.createByCluster = function(t, configuration) {
    var kmeanResults = clusterfck.kmeans(DataUtil.getInputData(t));
    var centers = [];
    for (var i = 0; i < kmeanResults.length; i++) {
        for (var j = 0; j < kmeanResults[i].length; j++) {
            centers.push(kmeanResults[i][j]);
        }
    }

    configuration.initialCenters = centers;
    configuration.hiddenLayerNeuronCount = centers.length;
    configuration.useBiasInput = false;
    return new RBFNetwork(configuration);
}
