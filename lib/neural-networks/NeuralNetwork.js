...
NeuralNetwork.prototype = {
    ...
    _adjustWeights: function() {
        for (var layerIdx = 0; layerIdx < this._layers.length - 1; layerIdx++) {
            var cLayer = this._layers[layerIdx];
            var nLayer = this._layers[layerIdx + 1];

            for (var nIdx = 0; nIdx < nLayer.length; nIdx++) {
                var n = nLayer[nIdx];

                for (var cIdx = 0; cIdx < cLayer.length; cIdx++) {
                    n.w[cIdx] += this.eta * n.error * n.dActivationFn(n.input) * cLayer[cIdx].output;
                }
            }
        }
    }
