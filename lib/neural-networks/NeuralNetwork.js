...
var NeuralNetwork = function(configuration) {
    ...
    this.alpha = 0.5;
    this.momentum = false;
    this.activationFunction = NetworkUtil.logisticFunction;
    this.dActivation = NetworkUtil.dLogisticFunction;
...
NeuralNetwork.prototype = {
    ...
    _feedForward: function() {
        ...
        for (var i = 1; i < this._layers.length - 1; i++) {
            ...
            for (cN = 0, cLen = curr.length; cN < cLen; cN++) {
                ...
                c.input = sum;
                c.output = this.activationFunction(sum);
            }
        }
    ...
    _backpropErrorCalculation: function(targetOutputs) {
        ...
        for (var hIdx = this._layers.length - 2; hIdx > 0; hIdx--) {
            var curr = this._layers[hIdx];
            var next = this._layers[hIdx + 1];
            ...
            for (var hN = 0, hLen = curr.length; hN < hLen; hN++) {
                ...
                h.error = sum * this.dActivation(h.output);
            }
        }
    },
        ...
        for (var cL = this._layers.length - 1; cL > 0; cL--) {
            ...
            for (var cN = 0, cLen = curr.length; cN < cLen; cN++) {
                ...
                for (var pN = 0, pLen = prev.length; pN < pLen; pN++) {
                    var delta = this.eta * c.error * prev[pN].output;

                    if (this.momentum) {
                        c.deltaW[pN] = delta + c.deltaW[pN] * this.alpha;
                    } else {
                        c.deltaW[pN] = delta;
                    }
