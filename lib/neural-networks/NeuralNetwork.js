...
NeuralNetwork.prototype = {
    ...
    _feedForward: function() {
        ...
        for (idx = 0, len = this._inputs.length; idx < len; idx++) {
            if (idx + 1 === len) {
                // The last node (bias node) is always activated
                this._inputs[idx].output = 1;
            } else {
                this._inputs[idx].output = this._inputs[idx].input;
            }
        }

        // The hidden layer calculation
        for (idx = 1, len = this._layers.length - 1; idx < len; idx++) {
            var layer = this._layers[idx];
            prevLayer = this._layers[idx - 1];
        ...
        for (idx = 0, len = this._outputs.length; idx < len; idx++) {
            var oN = this._outputs[idx];
