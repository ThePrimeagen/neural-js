...
var en = evo.firstFitOnePointRandomMutation(population, fit, 0.501, 0.35);
...
for (var i = 0; i < 10000; i++) {
    ...
    for (var j = population.length - 1; j >= 0; j--) {
        ...
        if (fit(population[j]) > fit(populationNext[j])) {
            finalList[j] = population[j];
        } else {
            finalList[j] = populationNext[j];
        }

        population.pop();
        population.unshift(populationNext[j]);
    }
}
