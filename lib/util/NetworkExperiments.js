...
function converge(network, t) {
    ...
    var error2 = [];
    var avg = [];
    var convergedLimit = network.count() * trials * 0.017;
    var avgConvergeLim = network.count() * 0.12;
    var out = 'Avg(' + network.name + '): ' + convergedLimit + ' :: ';
    ...
    while (!converged) {
        ...
        if (error2.length > 10) {
            ...
            if (tests % outAlert === 0) {
                console.log(outStr + ' :: ' + largestDiff);
            }
            if (Math.sqrt(largestDiff) < avgConvergeLim) {
                converged = true;
            }
            error2.shift();
        }
